...
var (
    ErrECBInvalidKeyLength = errors.New("length of the key is invalid")
    ErrECBInvalidValueSize = errors.New("source data must be an integer multiple of block size")
)

func GenerateMessageBasic(info MemoryUpdateInfo) (MemoryUpdateMessage, error) {

    var mum MemoryUpdateMessage
    ...
    mum, err = generateMessage(info, d1, d2)
    if err != nil {
        return mum, err
    }
...
func generateMessage(info MemoryUpdateInfo, KEY_UPDATE_ENC_C []byte, KEY_UPDATE_MAC_C []byte) (MemoryUpdateMessage, error) {
